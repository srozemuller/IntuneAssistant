...
// AUDIT_LOGS_INTUNE_FILTER is the only POST this context (or anything under app/audit-events) may
// call — it is allowlisted in lib/apiRequest.ts as a confirmed read: same Graph-read code path as
// GET /audit/intune/page, just carrying a body instead of a querystring for the extra
// activityType/actor/result filters that don't fit a querystring. Never add another verb/endpoint here.
export const AuditEventsProvider: React.FC<AuditEventsProviderProps> = ({ children }) => {
    ...
    const fetchData = useCallback(async (
        ...
        try {
            ...
            // Use filter endpoint (POST) if we need filtering, otherwise use GET page endpoint
            ...
            let response;
            if (shouldFilter) {
                const filterBody: Record<string, unknown> = { pageSize: 100 };
                if (filterType === 'failures') {
                    filterBody.activityResults = ['Failure'];
                }
                if (activity && activity !== 'all') {
                    filterBody.activityType = activity;
                }
                if (actor && actor !== 'all') {
                    filterBody.actor = actor;
                }
                response = await request<ApiEnvelope>(
                    AUDIT_LOGS_INTUNE_FILTER,
                    { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(filterBody) }
                );
            } else {
                response = await request<ApiEnvelope>(
                    `${AUDIT_LOGS_INTUNE_EVENTS}?${params.toString()}`,
            ...
            if (envelope?.status === 'Success' && envelope.data?.items) {
    ...
    const loadMore = useCallback(async () => {
        ...
        try {
            ...
            if (shouldFilter) {
                ...
                const filterUrl = nextPageToken
                    ? `${AUDIT_LOGS_INTUNE_FILTER}?skipToken=${encodeURIComponent(nextPageToken)}`
                    : AUDIT_LOGS_INTUNE_FILTER;
                response = await request<ApiEnvelope>(
                    filterUrl,
                    { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(filterBody) }
