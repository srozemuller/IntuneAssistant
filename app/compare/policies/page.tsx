...
export default function PolicyComparison() {
    ...
    const exportCSV = () => {
        ...
        const a = document.createElement('a'); a.href = url; a.download = buildExportFilename(exportTenantId, 'policy-decision-analysis', 'csv'); a.click();
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between">
                        ...
                        <Button variant="outline" size="sm" onClick={() => {
                            ...
                            const headers = Object.keys(rows[0]);
                            const csv = [headers.join(','), ...rows.map(r => headers.map(h => `"${(r[h as keyof typeof r] ?? '').replace(/"/g, '""')}"`).join(','))].join('\n');
                            const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([csv], { type: 'text/csv' })); a.download = buildExportFilename(exportTenantId, 'set-analysis', 'csv'); a.click();
                        }} className="gap-1.5">
