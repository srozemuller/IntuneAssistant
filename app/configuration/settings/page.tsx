...
import {
    ...
    Copy, Database, XCircle, Filter, Users, Archive, ArrowRightLeft, Check, Download
} from 'lucide-react';
import {
...
interface RawChildSettingInfo {
    name: string;
    value: string | null;
    children?: RawChildSettingInfo[] | null;
}

// Child setting info after flattening — value is always a string, children nest to any depth
interface ChildSettingInfo {
    name: string;
    value: string;
    children: ChildSettingInfo[] | null;
}
...
// One already-flattened, already-analyzed setting row, as returned by the backend
// policy-settings-analysis endpoint (duplicate/conflict detection runs server-side).
interface AnalysisRow {
    ...
    omaUri?: string | null;
    childSettingInfo: RawChildSettingInfo[] | null;
    ...
    conflictPeers: { policyId: string; policyName: string; settingValue: string; source: string; matchedAudiences: string[] }[];
}
...
// Recursively carries child settings through to any depth (children of children of...)
function mapChildSettingInfo(list: RawChildSettingInfo[] | null | undefined): ChildSettingInfo[] | null {
    const mapped = list
        ?.filter(c => c.name != null)
        .map(c => ({
            name: c.name,
            value: c.value ?? '',
            children: mapChildSettingInfo(c.children),
        })) ?? null;
    return mapped?.length ? mapped : null;
}

// Flattens a (possibly nested) child settings tree into "name: value" pairs for display text
function flattenChildSettingText(list: ChildSettingInfo[] | null): string[] {
    if (!list) return [];
    return list.flatMap(c => [
        ...(c.value ? [`${c.name}: ${c.value}`] : []),
        ...flattenChildSettingText(c.children),
    ]);
}

function countChildSettings(list: ChildSettingInfo[]): number {
    return list.reduce((sum, c) => sum + 1 + (c.children ? countChildSettings(c.children) : 0), 0);
}

function childSettingSelfMatches(c: ChildSettingInfo, searchLower: string): boolean {
    return c.name.toLowerCase().includes(searchLower) || c.value.toLowerCase().includes(searchLower);
}

// Finds every node (at any depth) that itself matches, or has a descendant that matches
function findMatchingChildSettings(list: ChildSettingInfo[], searchLower: string): ChildSettingInfo[] {
    const matches: ChildSettingInfo[] = [];
    for (const c of list) {
        const descendantMatches = c.children ? findMatchingChildSettings(c.children, searchLower) : [];
        if (childSettingSelfMatches(c, searchLower) || descendantMatches.length > 0) {
            matches.push(c);
        }
    }
    return matches;
...
function renderChildSettingNodes(list: ChildSettingInfo[], searchLower: string): React.ReactNode {
    return list.map((c, i) => {
        const isMatch = !!(searchLower && childSettingSelfMatches(c, searchLower));
        return (
            <div key={i} className={`text-xs rounded px-1 ${isMatch ? 'bg-amber-50 dark:bg-amber-900/20' : ''}`}>
                ...
                {c.children && c.children.length > 0 && (
                    <div className="ml-4 space-y-1 border-l-2 border-gray-200 pl-2 mt-1">
                        {renderChildSettingNodes(c.children, searchLower)}
                    </div>
                )}
            </div>
        );
...
// Duplicate/conflict detection (which policies share a setting, whether their assignment
// audiences actually overlap) runs server-side now — this just adapts the already-analyzed
// row shape into what the table/dialogs render.
function toFlatSetting(row: AnalysisRow): FlatSetting {
    const childInfo = mapChildSettingInfo(row.childSettingInfo);
    return {
        _id: `${row.source}-${row.policyId}-${row.settingId ?? row.omaUri ?? row.settingName}`,
        ...
        settingValue: row.settingValue || flattenChildSettingText(childInfo).join(', '),
        settingId: row.settingId ?? undefined,
        omaUri: row.omaUri ?? undefined,
        source: row.source,
...
interface PeersDialogInfo {
    ...
    peers: { policyId: string; policyName: string; settingValue: string; source: string; matchedAudiences: string[] }[];
}
                                ...
                                return (
                                    <div key={i} className={`p-3 rounded border ${valuesDiffer ? 'border-red-200 bg-red-50/50 dark:bg-red-900/10' : 'border-yellow-200 bg-yellow-50/50 dark:bg-yellow-900/10'}`}>
                                        ...
                                        <div className="text-xs text-muted-foreground mt-1.5 flex items-start gap-1">
                                            ...
                                            <span>
                                                {peer.matchedAudiences.length > 0
                                                    ? <>{isConflict ? 'Conflict' : 'Duplicate'} because both target {peer.matchedAudiences.join(', ')}</>
                                                    : <>Assignment scope unknown for one or both policies — flagged conservatively</>}
                                            </span>
                                        </div>
                                    </div>
                                );
...
// ── Migration Suggestion Dialog ───────────────────────────────────────────────

interface MigrationDialogInfo {
    settingName: string;
    omaUri?: string;
    info: OmaCatalogMigrationInfo;
}

function MigrationInfoDialog({ info, onClose }: { info: MigrationDialogInfo | null; onClose: () => void }) {
    const exportTenantId = useExportTenantId();
    const [copiedField, setCopiedField] = useState<'name' | 'json' | null>(null);

    useEffect(() => {
        if (!info) setCopiedField(null);
    }, [info]);

    if (!info) return null;
    const { info: migration } = info;

    const copyText = (text: string, field: 'name' | 'json') => {
    ...
    const downloadMigrationJson = () => {
        const safeName = migration.catalogDisplayName.replace(/[^a-z0-9]+/gi, '-').replace(/^-+|-+$/g, '').toLowerCase() || 'migrated-policy';
        const blob = new Blob([migration.migrationPolicyJson], { type: 'application/json;charset=utf-8;' });
        const link = document.createElement('a');
        const url = URL.createObjectURL(blob);
        link.setAttribute('href', url);
        link.setAttribute('download', buildExportFilename(exportTenantId, safeName, 'json'));
            ...
            <DialogContent className="max-w-2xl max-h-[70vh] overflow-y-auto">
                <DialogHeader>
                    <DialogTitle className="flex items-center gap-2">
                        ...
                        Settings Catalog Migration Available
                    </DialogTitle>
                    <DialogDescription>{migration.message}</DialogDescription>
                </DialogHeader>

                <div className="space-y-4">
                    <div>
                        <p className="text-xs font-medium text-muted-foreground mb-2">CURRENT OMA-URI SETTING</p>
                        <div className="p-3 rounded border bg-muted/30">
                            <div className="font-medium text-sm text-foreground">{info.settingName}</div>
                            {info.omaUri && (
                                <div className="text-xs font-mono text-muted-foreground mt-1 break-all">{info.omaUri}</div>
                            )}
                        </div>
                    </div>

                    <div>
                        <p className="text-xs font-medium text-muted-foreground mb-2">SETTINGS CATALOG EQUIVALENT</p>
                        <div className="p-3 rounded border-2 border-blue-300 bg-blue-50 dark:bg-blue-900/10 space-y-2">
                            <div className="flex items-center justify-between gap-2">
                                <div className="font-medium text-sm text-foreground">{migration.catalogDisplayName}</div>
                                <Button size="sm" variant="outline" className="h-7 text-xs shrink-0" onClick={() => copyText(migration.catalogDisplayName, 'name')}>
                                    {copiedField === 'name' ? <Check className="h-3 w-3 mr-1" /> : <Copy className="h-3 w-3 mr-1" />}
                                    {copiedField === 'name' ? 'Copied' : 'Copy name'}
                                </Button>
                            </div>
                            <p className="text-xs text-muted-foreground">
                                Search for this name in the Settings Catalog when creating a Configuration Policy.
                            </p>
                            {migration.catalogDescription && (
                                <p className="text-xs text-muted-foreground whitespace-pre-line">{migration.catalogDescription}</p>
                            )}
                        </div>
                    </div>

                    {migration.migrationPolicyJson && (
                        <div>
                            <div className="flex items-center justify-between mb-2">
                                <p className="text-xs font-medium text-muted-foreground">MIGRATED POLICY (JSON)</p>
                                <div className="flex items-center gap-2">
                                    <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => copyText(migration.migrationPolicyJson, 'json')}>
                                        {copiedField === 'json' ? <Check className="h-3 w-3 mr-1" /> : <Copy className="h-3 w-3 mr-1" />}
                                        {copiedField === 'json' ? 'Copied' : 'Copy'}
                                    </Button>
                                    <Button size="sm" variant="outline" className="h-7 text-xs" onClick={downloadMigrationJson}>
                                        <Download className="h-3 w-3 mr-1" /> Download
                                    </Button>
                                </div>
                            </div>
                            <pre className="text-xs bg-muted/50 border rounded p-3 overflow-x-auto max-h-64 whitespace-pre-wrap break-all">
                                {migration.migrationPolicyJson}
                            </pre>
                        </div>
                    )}
...
export default function PolicySettingsPage() {
    ...
    // Which sources to fetch — unchecking one skips it server-side entirely, so fetching is
    // faster when you only care about e.g. Settings Catalog.
    const [selectedSources, setSelectedSources] = useState<Set<string>>(
        new Set(['catalog', 'deviceconfig', 'grouppolicy', 'compliance'])
    );
    ...
    // Migration suggestion dialog
    const [migrationDialogInfo, setMigrationDialogInfo] = useState<MigrationDialogInfo | null>(null);
    // Group dialog
    ...
    const fetchSettings = useCallback(async () => {
        if (!accounts.length || selectedSources.size === 0) return;
        setLoading(true);
        setError(null);
        setIsCancelled(false);
        setSettings([]);
        try {
            const url = `${POLICY_SETTINGS_ANALYSIS_ENDPOINT}?sources=${Array.from(selectedSources).join(',')}`;
            const response = await request<AnalysisApiResponse>(url, {
                method: 'GET',
                headers: { 'Content-Type': 'application/json' },
            });
            const rows: AnalysisRow[] = response?.data?.data ?? [];
            setSettings(rows.map(toFlatSetting));
        } catch (err) {
    ...
    }, [accounts, request, selectedSources]);
    ...
    const statusOptions = useMemo<Option[]>(() => [
        ...
        { label: 'Migration Available', value: 'migratable' },
    ], []);
    ...
    const filteredSettings = useMemo(() => {
        ...
        if (statusFilter.length) {
            data = data.filter(s => {
                const regularStatuses = statusFilter.filter(f => f !== 'deprecated' && f !== 'migratable');
                if (regularStatuses.includes(s.status)) return true;
                if (statusFilter.includes('deprecated') && s.settingName.startsWith('[Deprecated]')) return true;
                if (statusFilter.includes('migratable') && !!s.catalogMigrationInfo) return true;
                return false;
            });
        }
    ...
    const stats = useMemo(() => ({
        ...
        compliance: settings.filter(s => s.source === 'compliance').length,
        ...
        migratable: settings.filter(s => !!s.catalogMigrationInfo).length,
    }), [settings]);
    ...
    const sourceBadgeClass = useMemo<Record<string, string>>(() => ({
        ...
        compliance: 'bg-amber-50 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300',
    }), []);
    ...
    const columns = useMemo(() => [
            ...
            render: (value: unknown, row: Record<string, unknown>) => {
                ...
                return (
                    <div className="space-y-1">
                        ...
                        {migrationInfo && (
                            <button onClick={() => setMigrationDialogInfo({ settingName: name, omaUri, info: migrationInfo })}>
                                <Badge variant="outline" className="text-xs bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 hover:bg-blue-100 cursor-pointer flex items-center gap-1">
                                    <ArrowRightLeft className="h-3 w-3" /> Migration Available
                                </Badge>
                            </button>
                        )}
                    </div>
        ...
        },
        {
            ...
            render: (value: unknown, row: Record<string, unknown>) => {
                ...
                const valueClass = status === 'conflict' ? 'text-red-600 dark:text-red-400 font-medium' : 'text-foreground';

                // Compliance non-compliance actions read better as a compact key/value list than
                // as one bullet-separated sentence — same data as childSettingInfo, formatted for
                // an always-visible glance instead of an expand-to-see interaction.
                if (source === 'compliance') {
                    const children = (row.childSettingInfo as ChildSettingInfo[] | null) ?? [];
                    return (
                        <div className="text-xs space-y-0.5" title={str}>
                            {children.map((c, i) => (
                                <div key={i} className="flex gap-1">
                                    <span className="text-muted-foreground shrink-0">{c.name}:</span>
                                    <span className={`truncate ${valueClass}`}>{c.value || '—'}</span>
                                </div>
                            ))}
                        </div>
                    );
                ...
                return (
                    <div className={`text-sm line-clamp-2 break-words ${status === 'conflict' ? valueClass : 'text-muted-foreground'}`} title={str}>
                        {str}
                    </div>
                );
            }
        ...
        },
        {
            ...
            minWidth: 120,
            render: (value: unknown, row: Record<string, unknown>) => {
                const children = value as ChildSettingInfo[] | null;
                const rowId = String(row._id ?? '');
                if (!children || children.length === 0) {
                    return <span className="text-xs text-muted-foreground">—</span>;
                }
                const searchLower = activeSearch.toLowerCase().trim();
                const matching = searchLower ? findMatchingChildSettings(children, searchLower) : [];
                const isExpanded = expandedRows.has(rowId) || matching.length > 0;
                const totalCount = countChildSettings(children);
                return (
                    <div className="space-y-1">
                        <button
                        ...
                        >
                            ...
                            {totalCount} child settings
                            {matching.length > 0 && <span className="text-amber-600 font-semibold">({matching.length} match{matching.length > 1 ? 'es' : ''})</span>}
                        </button>
                        {isExpanded && (
                            <div className="ml-4 space-y-1 border-l-2 border-gray-200 pl-2">
                                {renderChildSettingNodes(children, searchLower)}
                            </div>
                        )}
                    </div>
                );
    ...
    ], [activeSearch, expandedRows, sourceLabel, sourceBadgeClass, toggleRowExpansion, setPeersDialogInfo, setMigrationDialogInfo]);
    ...
    return (
        <div className="p-4 lg:p-8 space-y-6 w-full max-w-none">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p className="text-gray-600 dark:text-gray-300 mt-2">
                        View all settings across Settings Catalog, Device Configuration, Group Policy and Compliance
                    </p>
                </div>
                <div className="flex gap-2">
                    {settings.length > 0 ? (
                        <>
                            <Button onClick={fetchSettings} variant="outline" size="sm" disabled={loading || selectedSources.size === 0}>
                        ...
                        <>
                            <Button onClick={fetchSettings} disabled={loading || selectedSources.size === 0} className="flex items-center gap-2">
            ...
            {/* Source selection — which categories to fetch (unchecking one skips it server-side) */}
            <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm">
                <span className="text-muted-foreground font-medium">Categories to check:</span>
                ...
                {selectedSources.size === 0 && (
                    <span className="text-xs text-red-600 dark:text-red-400">Select at least one category</span>
                )}
            </div>
                    ...
                    <CardContent className="pt-6">
                        <div className="text-center py-12">
                            ...
                            <p className="text-gray-600 dark:text-gray-300 mb-6 max-w-md mx-auto">
                                Click &quot;Load Settings&quot; to fetch settings across the selected categories above.
                            </p>
                            <Button onClick={fetchSettings} disabled={selectedSources.size === 0} className="flex items-center gap-2 mx-auto" size="lg">
                    ...
                    <Card>
                        <CardContent className="p-4 space-y-3">
                            <div className="text-sm text-muted-foreground">
                                Fetching and analyzing settings across {Array.from(selectedSources).map(s => sourceLabel[s] ?? s).join(', ')}…
                            </div>
                    ...
                    <div className="flex flex-wrap gap-2 text-sm">
                        ...
                        {(
                            [
                                ...
                                { value: 'compliance',   count: stats.compliance,   label: 'Compliance',        cls: 'bg-amber-50 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300 border-amber-200 dark:border-amber-700' },
                            ] as const
                        ...
                        {/* Migration available badge */}
                        {stats.migratable > 0 && (() => {
                            const active = statusFilter.includes('migratable');
                            return (
                                <button
                                    onClick={() => setStatusFilter(active ? statusFilter.filter(f => f !== 'migratable') : [...statusFilter, 'migratable'])}
                                    title={active ? 'Remove migration filter' : 'Filter settings with a migration available'}
                                    className="focus:outline-none"
                                >
                                    <Badge
                                        variant="outline"
                                        className={`cursor-pointer hover:opacity-80 transition-opacity bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 border-blue-200 dark:border-blue-700 flex items-center gap-1 ${active ? 'ring-2 ring-offset-1 ring-blue-500' : ''}`}
                                    >
                                        <ArrowRightLeft className="h-3 w-3" /> {stats.migratable} migration{stats.migratable !== 1 ? 's' : ''} available
                                    </Badge>
                                </button>
                            );
                        })()}
            ...
            {settings.length > 0 && !error && (
                <DataTable
                    ...
                    searchExcludeKeys={['conflictPeers']}
            ...
            {/* Migration suggestion dialog */}
            <MigrationInfoDialog
                info={migrationDialogInfo}
                onClose={() => setMigrationDialogInfo(null)}
            />
