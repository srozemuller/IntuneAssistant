...
;
import { RefreshCw, Filter, Database, X, Users, ExternalLink, Settings, XCircle } from 'lucide-react';
import { CONFIGURATION_POLICIES_ENDPOINT, ASSIGNMENTS_FILTERS_ENDPOINT, ITEMS_PER_PAGE, GROUPS_LIST_ENDPOINT, ROLE_SCOPETAGS_ENDPOINT } from '@/lib/constants';
...
export default function ConfigurationPoliciesPage() {
    ...
    const scopeTagNameById = useMemo(() => {
        const map = new Map<string, string>();
        scopeTags.forEach(tag => map.set(tag.id, tag.displayName));
        return map;
    }, [scopeTags]);
    ...
    useEffect(() => {
        setCurrentPage(1);
    }, [policyTypeFilter, statusFilter, platformFilter, scopeTagFilter]);
    ...
    const prepareExportData = (): ExportData => {
        const exportColumns: ExportColumn[] = [
            ...
            },
            {
                ...
                width: 20,
                getValue: (row) => {
                    const tagIds = row.roleScopeTags as string[] | null;
                    if (!tagIds || tagIds.length === 0) return '';
                    return tagIds.map(id => scopeTagNameById.get(id) || id).join(', ');
                }
            },
            {
                key: 'createdDateTime',
    ...
    const fetchPolicies = async () => {
        ...
        try {
            await Promise.all([fetchPoliciesData(), fetchFilters(), fetchGroups(), fetchScopeTags()]);
        } catch (error) {
    ...
    const fetchScopeTags = async () => {
        ...
        try {
            const scopeTagsResponse = await request<{ status: string; message: string; details: unknown[]; data: RoleScopeTag[] }>(
                ROLE_SCOPETAGS_ENDPOINT,
                { method: 'GET', headers: { 'Content-Type': 'application/json' } }
            );

            if (scopeTagsResponse && Array.isArray(scopeTagsResponse.data.data)) {
                setScopeTags(scopeTagsResponse.data.data);
            } else {
                console.warn('Invalid scope tags response format:', scopeTagsResponse);
                setScopeTags([]);
            }
        } catch (error) {
            console.error('Failed to fetch scope tags:', error);
            setScopeTags([]);
        }
    };
    ...
    // Filter function (text search is handled by DataTable itself)
    useEffect(() => {
        ...
        if (scopeTagFilter.length > 0) {
            filtered = filtered.filter(policy => policy.roleScopeTags?.some(id => scopeTagFilter.includes(id)));
        }

        setFilteredPolicies(filtered);
    }, [policies, policyTypeFilter, statusFilter, platformFilter, scopeTagFilter]);
    ...
    const getUniqueScopeTags = (): Option[] => {
        const usedIds = new Set<string>();
        policies.forEach(policy => {
            policy.roleScopeTags?.forEach(id => usedIds.add(id));
        });
        return Array.from(usedIds)
            .map(id => ({ label: scopeTagNameById.get(id) || id, value: id }))
    ...
    const columns = useMemo(() => [
        ...
        {
            key: 'roleScopeTags' as string,
            label: 'Scope Tags',
            width: 160,
            minWidth: 120,
            render: (value: unknown) => {
                const tagIds = value as string[] | null;
                ...
                return (
                    <div className="flex flex-wrap gap-1">
                        {tagIds.map(id => (
                            <Badge key={id} variant="outline" className="text-xs whitespace-nowrap">
                                {scopeTagNameById.get(id) || id}
                            </Badge>
                        ))}
                    </div>
                );
    ...
    ], [groups, filters, scopeTagNameById]);
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground mt-2">
                        Browse the configuration policies in your tenant
                    </p>
                </div>
                    ...
                    <Card className="relative overflow-hidden transition-all duration-300 hover:shadow-2xl bg-white/60 dark:bg-gray-900/30 backdrop-blur-lg border border-white/30 dark:border-white/10">
                        ...
                        <CardContent className="space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                ...
                                {/* Scope Tag Filter */}
                                <div className="space-y-2">
                                    <label className="text-sm font-medium text-foreground">Scope Tags</label>
                                    <MultiSelect
                                        options={getUniqueScopeTags()}
                            ...
                            {(policyTypeFilter.length > 0 || statusFilter.length > 0 || platformFilter.length > 0 || scopeTagFilter.length > 0) && (
