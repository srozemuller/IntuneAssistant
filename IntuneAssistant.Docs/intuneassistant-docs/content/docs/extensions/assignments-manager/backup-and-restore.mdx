...
policy-backups_tenantID_2024-01-15.zip ├── compliance-policies/ │ ├── windows-security-baseline.json │ ├── ios-compliance-policy.json │ └── android-app-protection.json ├── configuration-profiles/ │ ├── wifi-corporate.json │ └── vpn-settings.json └── app-protection/ ├── office-365-protection.json └── corporate-apps-policy.json
...
- Always create a fresh backup
- Include a descriptive timestamp
- Store in a secure, accessible location

**Regular backups:**
- Weekly automated backups for active environments
- Before major organizational changes
- Prior to policy rollouts or updates
...
- Verify sufficient permissions in Intune
