...
public sealed class AppsService : IAppsService
{
    ...
    public async Task<List<MobileAppDependencyModel>?> GetAppDependenciesListAsync(string? accessToken)
    {
        ...
        try
        {
            ...
            foreach (var application in applications)
            {
                try
                {
                    var response = await _http.GetAsync($"{GraphUrls.MobileAppsUrl}/{application.Id}/relationships?$filter=(isof(%27microsoft.graph.mobileAppDependency%27))");
                    var responseStream = await response.Content.ReadAsStreamAsync();
                    ...
                    var content = await sr.ReadToEndAsync();
                    // Deserialize the string to your model
                    var dependencies = JsonConvert.DeserializeObject<GraphValueResponse<MobileAppDependencyResponseModel>>(content);
    ...
    public async Task<List<MobileAppDependencyModel>?> GetAppDependenciesByAppNameAsync(string accessToken, string applicationDisplayName)
    {
         _http.DefaultRequestHeaders.Clear();
        _http.DefaultRequestHeaders.Add("Authorization", $"Bearer {accessToken}");
        var applications = new List<WindowsLobAppModel>();
        ...
        try
        {
            ...
            while (nextUrl is not null)
            {
                try
                {
                    ...
                    var dependenciesResponse = await _http.GetAsync($"{GraphUrls.MobileAppsUrl}/{application.Id}/relationships?$filter=(isof(%27microsoft.graph.mobileAppDependency%27))");
                    ...
                    // Deserialize the string to your model
                    var dependencies = JsonConvert.DeserializeObject<GraphValueResponse<MobileAppDependencyResponseModel>>(dependenciesContent);
                    if (dependencies?.Value is null)
                    {
                        nextUrl = null;
                        continue;
